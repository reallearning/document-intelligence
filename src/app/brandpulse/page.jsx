    ...
    insights: [
      {
        ...
        evidence: [
          ...
          { source: "Seasonal Calendar", summary: "Wedding season April–May drives 15–20% antacid uplift. Sachet format 3x more important. If not countered, Gas-O-Fast captures seasonal demand." },
        ],
  ...
  Otrivin: {
    ...
    insights: [
      ...
      },
      {
        ...
        recommendation: "Place pre-emptive top-up orders with Bhopal, Indore, Jabalpur, Gwalior by Wednesday. Request 15% buffer stock for all MP distributors. Prepare early-activation monsoon digital campaign (week of Jun 15 vs normal Jun 25). Push to MP field teams: stock critically low, place orders immediately.",
        agentTrail: [
    ...
    <div className="overflow-auto" style={{ fontFamily: "'DM Sans', 'Helvetica Neue', sans-serif", background: C.page, height: "100vh", color: C.text }}>
      ...
      <div style={{ maxWidth: 1180, margin: "0 auto", padding: "28px 40px 60px" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 28 }}>
          {brandList.map(b => (
            <div key={b} onClick={() => { setSelectedBrand(b); setExpandedInsight(null); setExpandedRegion(null); }}
              style={{
                padding: "10px 20px", cursor: "pointer", fontSize: 13, fontWeight: selectedBrand === b ? 700 : 500,
                color: selectedBrand === b ? C.card : C.text,
                background: selectedBrand === b ? C.green : C.card,
                border: `1px solid ${selectedBrand === b ? C.green : C.border}`,
                transition: "all 0.15s",
              }}
              onMouseEnter={e => { if (selectedBrand !== b) e.currentTarget.style.borderColor = C.sage; }}
              onMouseLeave={e => { if (selectedBrand !== b) e.currentTarget.style.borderColor = C.border; }}>
              {b}
            </div>
          ))}
        </div>
