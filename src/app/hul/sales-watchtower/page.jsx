...
export default function HULSalesWatchtower() {
  ...
  const dataSources = {
    primary: { label: 'Primary Sales (SAP)', lag: 'T+1', color: C.sage },
    shikhar: { label: 'Shikhar App', lag: 'T+0', color: C.sage },
    nielsen: { label: 'Nielsen IQ', lag: 'T+6w', color: '#946B1A' },
    ecom: { label: 'Ecom Scrape', lag: 'T+0', color: C.sage },
    scheme: { label: 'Scheme Mgmt', lag: 'T+1', color: C.sage },
  };

  const SourceBadge = ({ id }) => {
    const s = dataSources[id];
    if (!s) return null;
    const isLagged = id === 'nielsen';
    return <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '2px 8px', borderRadius: 4, background: isLagged ? C.amberSoft : C.lightSage, fontSize: 9, fontFamily: mono || font, letterSpacing: 0.3 }}><span style={{ width: 4, height: 4, borderRadius: '50%', background: s.color }} /><span style={{ color: C.darkGrey }}>{s.label}</span><span style={{ color: isLagged ? C.amber : C.lightGrey }}>{s.lag}</span></span>;
  };

  const sidebarWidth = 64;

  const notifications = [
    { id: 1, text: 'Surf Excel 1kg pipeline alert — Bareilly distributors loading down 42%', time: '12 min ago', unread: true },
    { id: 2, text: 'Weekly primary achievement report generated — region at 82%', time: '1 hr ago', unread: true },
    { id: 3, text: 'Gorakhpur territory at 104% — benchmark behavior documented', time: '3 hrs ago', unread: false },
  ];

  const briefingPoints = [
    {
      type: 'critical',
      title: 'Surf Excel 1kg pipeline drying — 3 Bareilly distributors primary loading down 42% this week',
      detail: 'Primary billing alert: Sharma Trading, Verma Agencies, and Gupta Sales placed 42% fewer cases of Surf Excel 1kg this week (2,480 cases vs 4,280 typical). These 3 distributors collectively serve 820 GT outlets. At current distributor inventory levels (estimated 4-5 days cover based on typical secondary offtake), stockout risk emerges by end of next week. This is the #1 Home Care SKU by GSV — primary loading velocity is the earliest signal we have.',
      sources: ['primary']
    },
    {
      type: 'alert',
      title: 'Dove shampoo 180ml — 4 distributors zero primary billing in last 15 days',
      detail: 'Primary sales data shows zero orders for Dove 180ml from Singh Distributors (Moradabad), Tiwari Agency (Lucknow East), Mishra Trading (Sultanpur), and Pandey Sales (Ayodhya). These 4 distributors cover 1,180 outlets. Last billing: Jan 28 for Singh, Feb 1 for the other three. Either they are sitting on unsold stock (forward-buy from the Jan scheme?) or they have delisted the SKU from routine orders. Need ASM confirmation on ground reality.',
      sources: ['primary']
    },
    {
      type: 'alert',
      title: 'Trade scheme TM-Q4-08 (Vim bar slab) showing forward-buy pattern at 2 distributors',
      detail: 'Sharma Trading and Gupta Sales loaded 3.2x normal Vim 250g quantity in week 1 of the slab scheme (buy 15 cases get 2 free). This pattern matches classic forward-buying — they will claim the slab benefit and destock slowly over 4-5 weeks, depressing genuine demand signals. If this continues, scheme ROI will be overstated and the Rs 18L allocated will generate subsidized loading, not incremental distribution.',
      sources: ['primary', 'scheme']
    },
    {
      type: 'performance',
      title: 'Region primary achievement at 82% MTD vs target — Rs 4.0Cr gap, 9 working days left',
      detail: 'Current MTD primary billing: Rs 18.0Cr of Rs 22.0Cr target. Bareilly territory at 72% (Rs 2.1Cr gap), Lucknow at 84%, Gorakhpur at 104%, Ayodhya at 89%. Closing the gap requires Rs 44L/day run-rate vs current Rs 36L/day. Gorakhpur is overachieving at Rs 5.5Cr MTD but cannot compensate for Bareilly deficit. The shortfall is concentrated in Home Care portfolio (Surf Excel, Vim, Rin) at Bareilly distributors.',
      sources: ['primary']
    },
    {
      type: 'highlight',
      title: 'Gorakhpur territory at 104% — Yadav Distribution replicating combo slab across full BPC portfolio',
      detail: 'Yadav Distribution in Gorakhpur adopted the Surf Excel slab structure for Dove, Pears, and Lifebuoy — ordering in slab-aligned quantities without a formal scheme. Their primary billing is up 16% MoM with consistent weekly cadence (no loading spikes). This is genuine demand-pull behavior worth understanding and replicating. ASM Gorakhpur reports the distributor principal personally reviews weekly order plans with the Shikhar app.',
      sources: ['primary', 'shikhar']
    }
  ];
  ...
  const insights = [
    {
      ...
      category: 'bpc',
      priority: 'critical',
      title: 'BPC portfolio distribution gap of 24 points vs core portfolio - 1,240 unbilled outlets',
      summary: 'Dove and Rin bar 250g at 68% numeric distribution vs 92% for Surf Excel - Rs 2.2Cr quarterly gap',
      keyNumbers: {
      ...
      narrative: "BPC SKU numeric distribution has declined 8 points vs last year same month, now at 68% compared to 92% for core Surf Excel portfolio. Bareilly District shows steepest drop at 61% distribution (down from 74% LYSM). Analysis reveals systematic pattern: 4 of 18 distributors consistently under-stock BPC portfolios despite maintaining 85%+ core distribution. These distributors collectively serve 1,240 outlets - all currently unbilled for BPC portfolio. Distributor order frequency data shows BPC portfolios ordered 18% less frequently than core SKUs, suggesting systematic deprioritization in primary billing cycle.",
      dataTable: {
        headers: ['Territory', 'BPC Dist', 'vs LYSM', 'Unbilled Outlets', 'Quarterly Gap'],
        rows: [
      ...
      recommendation: {
        action: "Systematic distributor engagement required to restore BPC portfolio order frequency. Focus on 4 distributors serving 1,240 unbilled outlets. Address root causes: order size constraints and beat coverage gaps for BPC portfolio.",
      ...
      owner: 'Distribution Planning - North India Region',
      color: C.sage,
      sources: ['primary', 'shikhar'],
      trail: [
        {
          ...
          database: 'SAP COPA',
          query: 'SELECT territory, sku_category, numeric_distribution, outlet_count, trend_vs_lysm FROM weekly_distribution WHERE region = "North_Region" AND week = CURRENT_WEEK',
          thinking: 'BPC portfolio distribution at 68% vs 92% for core portfolio - 24 point gap. Lucknow territory showing steepest decline at 61% (down 13 points vs LYSM). This gap is widening month-over-month.',
        ...
        },
        {
          ...
          database: 'SAP Primary Sales',
          query: 'SELECT distributor_id, distributor_name, bpc_sku_numeric_dist, core_sku_numeric_dist, outlet_count, unbilled_outlets FROM distributor_performance WHERE territory IN ("Bareilly", "Moradabad", "Ayodhya", "Gorakhpur") AND bpc_sku_numeric_dist < 70',
          thinking: '4 distributors maintain 85%+ core distribution but only 60-65% premium. Collectively serving 1,240 outlets that are unbilled for BPC portfolio. This is a systematic stocking selectivity pattern.',
          nextAgent: 'Sales Agent'
        },
        {
          timestamp: '09:17 AM',
          agent: 'Sales Agent',
          action: 'Examining primary billing order frequency patterns',
          database: 'SAP COPA',
          query: 'SELECT distributor_id, sku_category, avg_order_frequency, orders_per_month, trend_3_months FROM primary_sales WHERE distributor_id IN (102, 105, 108, 112) AND month = CURRENT_MONTH',
          thinking: 'BPC SKU order frequency 18% lower than core at these 4 distributors. Ordering pattern shows 2 times per month for BPC vs 4 times per month for core. Gap indicates systematic deprioritization in primary billing cycle.',
          nextAgent: 'Distribution Agent'
        },
        {
          timestamp: '09:19 AM',
          agent: 'Distribution Agent',
          action: 'Analyzing unbilled outlet coverage for BPC portfolios',
          database: 'SAP Primary Sales',
          query: 'SELECT territory, COUNT(outlet_id) as unbilled_outlets, SUM(potential_monthly_value) as opportunity FROM outlet_billing_patterns WHERE bpc_sku_billed = 0 AND core_sku_billed = 1 GROUP BY territory',
          thinking: '1,240 outlets currently unbilled for BPC portfolios despite ordering core portfolio regularly. Bareilly District accounts for 485 of these outlets (Rs 85L quarterly opportunity). This is a coverage execution gap.',
        ...
        },
        {
          ...
          query: 'SELECT sku_category, current_distribution, lysm_distribution, gap_value_quarterly, trend_direction FROM distribution_gap_analysis WHERE region = "North_Region" AND category = "bpc"',
          thinking: 'BPC portfolio gap widened from 18 points (LYSM) to 24 points currently. If gap continues expanding at current rate, quarterly opportunity loss reaches Rs 2.8Cr by next quarter. Distribution recovery to 84% unlocks Rs 1.6Cr recurring quarterly revenue.',
          nextAgent: null
        }
      ],
      whatIfScenarios: [
        'What if BPC portfolio gap continues widening at current rate?',
      ...
      trail: [
        {
          ...
          database: 'SAP COPA',
          query: 'SELECT territory, numeric_distribution, lysm_distribution, outlet_service_frequency, distributor_count FROM territory_performance WHERE region = "North_Region" ORDER BY numeric_distribution DESC',
        ...
        },
        {
          ...
          database: 'SAP Primary Sales',
          query: 'SELECT territory, AVG(service_frequency_weekly) as avg_service_freq, COUNT(CASE WHEN service_pattern = "sporadic" THEN 1 END) as sporadic_outlets FROM outlet_service_patterns WHERE region = "North_Region" GROUP BY territory',
        ...
        },
        {
          ...
          database: 'SAP COPA',
          query: 'SELECT territory, distributor_id, AVG(weekly_order_frequency) as order_freq, trend_3_months FROM distributor_ordering_patterns WHERE region = "North_Region" GROUP BY territory',
        ...
        },
        {
          ...
          database: 'Analytics DB',
          query: 'SELECT territory, current_distribution, target_distribution, gap_value_annually, outlet_count FROM distribution_opportunity WHERE region = "North_Region" ORDER BY gap_value_annually DESC',
    ...
    },
    {
      ...
      category: 'summer',
      priority: 'high',
      title: 'Seasonal portfolio numeric distribution at 72% vs 88% target - 18 days to seasonal peak demand',
      ...
      keyNumbers: {
        ...
        gapVsLYSM: '-6 pts vs Festive season 2025'
      },
      narrative: "Seasonal portfolio (seasonal gift combos, festive packs, Dove BPC travel kits) currently at 72% numeric distribution vs 88% target for festive/wedding season. This represents a 6-point decline vs last year's seasonal pre-positioning. Distribution gap concentrated in Moradabad District (64%) and Bareilly District (68%), while Ayodhya District and Gorakhpur track closer to target at 78-82%. Secondary sales data shows 615 outlets have not been billed for seasonal SKUs despite having regular orders for core portfolio. Seasonal portfolio order frequency from distributors is 2 times per week vs target of 2-3 times - indicating insufficient push in primary billing cycle. Historical correlation shows 88%+ distribution during festive season drives 34% volume uplift.",
      dataTable: {
        headers: ['Territory', 'Seasonal portfolio Dist', 'vs LYSM', 'Unbilled Outlets', 'Gap to Target'],
        rows: [
      ...
      supplyChainBacking: "Seasonal portfolio inventory at 142% of normal levels. No supply constraints.",
      recommendation: {
        action: "Accelerate seasonal peak SKU primary billing push across 615 unbilled outlets. Focus distributor engagement in Bareilly and Lucknow territories to increase order frequency from 2 times to 2-3 times per week. Priority on outlets with high footfall during seasonal peak period.",
        return: "Achieving 88% distribution protects Rs 88L festive season revenue opportunity",
        roi: "Summer peaks contribute 12-15% of quarterly revenue in North India region"
      },
      owner: 'Distribution Planning - Summer Peak Category',
      color: C.sage,
      sources: ['primary', 'primary', 'shikhar'],
      trail: [
        {
          timestamp: '11:20 AM',
          agent: 'Orchestrator Agent',
          action: 'Analyzing seasonal peak SKU distribution readiness',
          database: 'SAP COPA',
          query: 'SELECT season_name, sku_category, current_distribution, target_distribution, days_to_peak FROM summer_distribution_tracking WHERE region = "North_Region" AND peak_date > CURRENT_DATE ORDER BY peak_date LIMIT 1',
          thinking: 'seasonal peak in 18 days. Seasonal portfolio at 72% numeric distribution vs 88% target (16 point gap). This is 6 points below LYSM positioning for same period. Distribution lag concentrated in certain territories.',
          nextAgent: 'Distribution Agent'
        },
        {
          timestamp: '11:22 AM',
          agent: 'Distribution Agent',
          action: 'Mapping seasonal peak SKU stocking patterns by territory',
          database: 'SAP Primary Sales',
          query: 'SELECT territory, COUNT(outlet_id) as total_outlets, SUM(CASE WHEN summer_sku_billed = 1 THEN 1 ELSE 0 END) as stocked_outlets, SUM(CASE WHEN summer_sku_billed = 0 AND core_sku_billed = 1 THEN 1 ELSE 0 END) as unbilled_outlets FROM outlet_billing_patterns WHERE region = "North_Region" GROUP BY territory',
          thinking: '615 outlets unbilled for seasonal SKUs despite regular core portfolio orders. Moradabad District accounts for 288 unbilled outlets (64% distribution), Bareilly 352 outlets (68% distribution). Gap vs Ayodhya District (82%) and Gorakhpur (78%) indicates execution variance.',
          nextAgent: 'Sales Agent'
        },
        {
          timestamp: '11:24 AM',
          agent: 'Sales Agent',
          action: 'Examining seasonal peak SKU primary billing velocity',
          database: 'SAP COPA',
          query: 'SELECT territory, distributor_id, AVG(summer_sku_order_frequency) as order_freq, trend_vs_lysm FROM distributor_ordering_patterns WHERE sku_category = "summer_peak" AND region = "North_Region" GROUP BY territory',
          thinking: 'Seasonal portfolio order frequency from distributors at 2 times per week vs target of 2-3 times. This ordering rhythm gap drives the distribution shortfall. Top territories maintain 2.3-2-3 times frequency, lagging territories at 1.5-2 times.',
          nextAgent: 'Distribution Agent'
        },
        {
          timestamp: '11:26 AM',
          agent: 'Distribution Agent',
          action: 'Analyzing historical seasonal peak period performance correlation',
          database: 'SAP COPA',
          ...
          thinking: 'Historical correlation shows 88%+ seasonal peak SKU distribution drives 34% volume uplift during 2-week seasonal peak window. Last Summer Peak: territories below 75% distribution lost Rs 92L across North India region. Current 72% distribution projects Rs 88L opportunity gap.',
          nextAgent: 'Analytics Engine'
        },
        {
          timestamp: '11:28 AM',
          ...
          database: 'Analytics DB',
          query: 'SELECT territory, current_distribution, target_distribution, unbilled_outlets, required_order_frequency_increase, revenue_protection FROM summer_gap_analysis WHERE region = "North_Region" AND season = "Summer_2026"',
          thinking: 'Achieving 88% distribution requires converting 615 unbilled outlets and increasing distributor order frequency from 2 times to 2-3 times per week in Bareilly and Lucknow territories. This protects Rs 88L festive season revenue opportunity.',
          nextAgent: null
        }
      ],
      whatIfScenarios: [
        'What if seasonal peak SKU distribution remains at 72% through seasonal peak period?',
        'How does distributor order frequency impact seasonal peak SKU reach?',
    ...
    },
    {
      ...
      category: 'display',
      priority: 'healthy',
      title: 'Gorakhpur flagship outlets billing 28% higher than non-display outlets — asset ROI proven but underexploited',
      summary: 'Display-equipped outlets across the region generate Rs 11,400/month vs Rs 8,900 at non-display outlets, but Bareilly flagship outlet velocity is declining',
      keyNumbers: {
        displayLift: '+28% billing velocity',
        displayOutlets: '1,840 region-wide',
        bareillyDecline: '-14% velocity drop L3M',
        gorakhpurBenchmark: 'Rs 12,200/outlet/mo'
      },
      narrative: "Cross-referencing the asset master (display placements) with SAP billing data reveals a clear pattern: flagship outlets generate 28% higher monthly billing than non-display outlets in the same beats (Rs 11,400 vs Rs 8,900 region average). However, this lift varies sharply by territory. Gorakhpur flagship outlets average Rs 12,200/month with stable month-on-month velocity. Bareilly flagship outlets have dropped from Rs 10,800 to Rs 9,300 over the last 3 months — a 14% velocity decline that is not mirrored at non-display outlets in the same beats. This suggests the display asset itself is being underutilized: likely poor stocking, reduced salesman attention, or competitive encroachment at flagship outlets specifically. Bareilly has 455 flagship outlets, and the velocity gap between Bareilly (Rs 9,300) and Gorakhpur (Rs 12,200) represents Rs 13.2L/month in unrealized throughput from existing assets alone.",
      dataTable: {
        headers: ['Territory', 'Display Outlet Velocity', 'vs L3M Trend', 'Non-display Velocity', 'Display Outlets'],
        rows: [
      ...
      recommendation: {
        action: "Investigate the Bareilly display velocity decline: cross-reference with salesman visit data at flagship outlets, check if flagship outlets are being visited less frequently than non-display outlets in the same beat. Gorakhpur ASM reports that frequent display-specific attention during visits drives the velocity gap — worth validating and replicating. Consider deploying a display-focused visit protocol as a pilot in Bareilly.",
        return: "Closing the velocity gap between Bareilly (Rs 9,300) and Gorakhpur (Rs 12,200) across 455 flagship outlets = Rs 13.2L/month uplift",
      ...
      owner: 'Trade Marketing - Display Assets',
      color: C.sage,
      sources: ['primary'],
      trail: [
        { timestamp: '08:30 AM', agent: 'Orchestrator Agent', action: 'Comparing outlet billing velocity by display status', database: 'SAP Primary Sales + Shikhar Asset DB', query: 'SELECT territory, has_display, AVG(monthly_billing) as avg_velocity, COUNT(outlet_id) as outlet_count FROM outlet_billing JOIN asset_master ON outlet_id WHERE region = "North_Region" AND billing_month >= DATE_SUB(CURRENT_DATE, INTERVAL 3 MONTH) GROUP BY territory, has_display', thinking: 'Flagship outlets bill 28% higher than non-display across the region. But the gap varies: Gorakhpur +30%, Bareilly only +4%. Bareilly flagship outlets are underperforming their asset potential.', nextAgent: 'Distribution Agent' },
        { timestamp: '08:32 AM', agent: 'Distribution Agent', action: 'Analyzing flagship outlet velocity trend by territory', database: 'SAP Primary Sales', query: 'SELECT territory, month, AVG(monthly_billing) as velocity FROM outlet_billing JOIN asset_master ON outlet_id WHERE has_display = 1 AND region = "North_Region" GROUP BY territory, month ORDER BY month DESC LIMIT 12', thinking: 'Gorakhpur flagship outlets: stable at Rs 12,200 for 6 months. Bareilly flagship outlets: Rs 10,800 three months ago, Rs 9,300 now. 14% decline. Non-flagship outlets in Bareilly are flat. The decline is display-specific, not territory-wide.', nextAgent: 'Sales Agent' },
        { timestamp: '08:34 AM', agent: 'Sales Agent', action: 'Checking salesman visit patterns at display vs non-display outlets', database: 'Shikhar App', query: 'SELECT territory, has_display, AVG(visits_per_month) as visit_freq, AVG(time_per_visit_mins) as visit_duration FROM salesman_visits JOIN asset_master ON outlet_id WHERE territory IN ("Bareilly-Moradabad", "Gorakhpur-Deoria") GROUP BY territory, has_display', thinking: 'Gorakhpur salesmen spend 18% more time at flagship outlets than non-display (8.2 min vs 6.9 min). Bareilly salesmen show no difference (6.4 min vs 6.2 min). Gorakhpur salesmen are doing something extra at flagship outlets that Bareilly salesmen are not.', nextAgent: 'Analytics Engine' },
        { timestamp: '08:36 AM', agent: 'Analytics Engine', action: 'Quantifying display velocity recovery opportunity', database: 'Analytics DB', query: 'SELECT territory, display_outlets, current_velocity, gorakhpur_benchmark, monthly_gap, annual_opportunity FROM display_opportunity_model WHERE region = "North_Region"', thinking: 'Closing Bareilly display velocity to Gorakhpur benchmark: 455 outlets × Rs 2,900/month gap = Rs 13.2L/month = Rs 1.6Cr/year. This is an execution fix on existing assets, not new capex. The Shikhar visit data suggests the lever is salesman behavior at flagship outlets.', nextAgent: null }
      ],
      whatIfScenarios: [
        'What is driving the velocity decline at Bareilly flagship outlets specifically?',
        'How much time do Gorakhpur salesmen spend at flagship outlets vs Bareilly?',
        'Would adding new displays in Lucknow outperform fixing velocity on existing Bareilly displays?',
        'What is the correlation between flagship outlet velocity and competitive billing patterns?'
      ]
    },
    {
      id: 5,
      ...
      summary: 'Clustered, simultaneous primary billing drops across 680 outlets in Bareilly City and Rampur, not explained by seasonal or distribution factors',
      keyNumbers: {
      ...
      narrative: "SAP billing analysis flagged an unusual pattern: 14 beats in Bareilly City and Rampur show synchronized primary billing decline of 15-22% over the last 8 weeks. This is not a gradual territory-wide trend — adjacent beats in the same distributor territories are stable or growing. The decline is concentrated in mid-tier kirana outlets (Rs 400-800/week billing), not large accounts or modern trade. Order frequency at affected outlets dropped from 3.2 to 2.4 orders per month, and average order value declined 12%. Nielsen Western UP data for the same period shows ITC gaining 2.8 share points in fabric wash in Western UP. The geographic clustering (14 contiguous beats), outlet profile (mid-tier GT), and timing (simultaneous onset) are consistent with coordinated competitive entry, not organic churn. Gorakhpur territory shows no similar pattern despite comparable outlet profiles, suggesting visit frequency (3x/week in Gorakhpur vs 2x in Bareilly) may be a protective factor.",
      dataTable: {
      ...
      recommendation: {
        action: "Immediate field investigation: deploy ASM Bareilly for 2-day outlet audit in the 14 affected beats to confirm competitive cause (ITC presence, rack placements, pricing changes). In parallel, increase salesman visit frequency from 2x to 3x/week in affected beats and activate a retention scheme (Rs 15/case margin bump for 4 weeks) at the 680 affected outlets to stem further volume loss while root cause is confirmed.",
      ...
      sources: ['primary'],
      trail: [
        { timestamp: '09:45 AM', agent: 'Orchestrator Agent', action: 'Detecting anomalous billing decline pattern', database: 'SAP Primary Sales', query: 'SELECT beat_id, beat_name, pct_change_8_weeks, outlet_count, weekly_billing FROM beat_performance WHERE territory = "Bareilly-Moradabad" AND pct_change_8_weeks < -15 ORDER BY pct_change_8_weeks ASC', thinking: '14 beats with >15% decline in 8 weeks. Adjacent beats are stable. This is geographically clustered and simultaneous — not random outlet-level churn. Pattern suggests coordinated external factor.', nextAgent: 'Distribution Agent' },
        { timestamp: '09:47 AM', agent: 'Distribution Agent', action: 'Ruling out distribution and supply causes', database: 'SAP Primary Sales', query: 'SELECT beat_id, distributor_stock_cover, distributor_order_freq, salesman_visit_pct FROM beat_health_check WHERE beat_id IN (SELECT beat_id FROM beat_performance WHERE pct_change_8_weeks < -15)', thinking: 'Distributor stock levels and order frequency are normal in affected beats. Salesman visit rates are unchanged. This rules out supply-side causes. The decline is demand-side — outlets are ordering less despite normal supply availability.', nextAgent: 'Sales Agent' },
        { timestamp: '09:49 AM', agent: 'Sales Agent', action: 'Profiling affected outlet characteristics', database: 'SAP Primary Sales', query: 'SELECT outlet_tier, avg_weekly_billing_before, avg_weekly_billing_current, order_frequency_change, lines_per_order_change FROM outlet_decline_analysis WHERE beat_id IN (affected_beats) GROUP BY outlet_tier', thinking: 'Decline concentrated in mid-tier outlets (Rs 400-800/week). Large accounts and MT stable. Mid-tier kirana are the most vulnerable to competitive switching — they lack loyalty commitments and respond to margin offers. Profile matches competitive displacement pattern.', nextAgent: 'Analytics Engine' },
        { timestamp: '09:51 AM', agent: 'Analytics Engine', action: 'Cross-referencing with Nielsen market data', database: 'Analytics DB + Nielsen IQ', query: 'SELECT geography, brand, share_change_8_weeks FROM nielsen_share WHERE geography = "Western_UP" AND category = "fabric wash" AND share_change > 2', thinking: 'Nielsen shows ITC gaining 4.2 share points in Western UP fabric wash segment over the same 8-week period. Geographic and temporal correlation with the SAP billing decline. Combined evidence strongly suggests competitive displacement, though field confirmation is needed.', nextAgent: null }
      ],
      whatIfScenarios: [
    ...
    },
    {
      ...
      category: 'tradepromo',
      priority: 'medium',
      title: 'Trade promo ROI varies 4x across schemes — top scheme drives 8.2 GSV per Rs spent vs bottom at 2.1 GSV per Rs spent',
      summary: 'Slab-based volume schemes outperforming flat margin schemes by 3.2x on cases-per-rupee metric',
      keyNumbers: {
        topSchemeROI: '8.2 GSV per Rs spent',
        bottomSchemeROI: '2.1 GSV per Rs spent',
        activeSchemes: '7 concurrent',
        totalSpend: 'Rs 3.8Cr MTD'
      },
      narrative: "Analysis of 7 concurrent trade promotion schemes running in the North India region reveals significant ROI variance. The top performer is the Surf Excel volume slab scheme (buy 10 cases get 1 free equivalent) at 8.2 GSV per Rs spent invested — this scheme drives incremental ordering behavior because distributors actively chase the slab threshold. The worst performer is the flat Rs 8/case margin boost on Rin bar 250g at 2.1 GSV per Rs spent — this scheme is not changing distributor behavior, it is simply subsidizing existing volume. Three schemes (Dove seasonal push, Vim combo, and Pears margin boost) are performing in the middle range (4.5-5.8 GSV per Rs spent). The ITC/competitors counter-scheme (emergency margin match in Bareilly) is at 3.4 GSV per Rs spent but is defensive spending — its purpose is volume retention, not growth.",
      dataTable: {
        headers: ['Scheme', 'Type', 'MTD Spend', 'GSV per Rs Spent', 'Incremental Volume'],
        rows: [
          ['Surf Excel Volume Slab', 'Slab', 'Rs 82L', '8.2', '+6,700 cases'],
          ['Dove Seasonal Push', 'Combo', 'Rs 48L', '5.8', '+2,780 cases'],
          ['Vim Display Combo', 'Combo', 'Rs 35L', '5.1', '+1,785 cases'],
          ['Pears Margin Boost', 'Flat margin', 'Rs 42L', '4.5', '+1,890 cases'],
          ['ITC Share Defense', 'Defensive', 'Rs 68L', '3.4', '+2,312 cases'],
          ['BPC Bundle Push', 'Bundle', 'Rs 55L', '2.8', '+1,540 cases'],
          ['Rin bar 250g Margin Boost', 'Flat margin', 'Rs 50L', '2.1', '+1,050 cases']
        ]
      },
      recommendation: {
        action: "Reallocate Rs 50L from the two underperforming flat margin schemes (Rin bar 250g, BPC bundle) into an expanded Surf Excel slab structure and a new Dove-Rin combo slab. Slab mechanics drive behavior change; flat margins just subsidize status quo. Retain ITC share-defense scheme as defensive necessity.",
      ...
      owner: 'Trade Marketing - SAP Scheme Mgmt',
      color: C.sage,
      sources: ['primary', 'primary'],
      trail: [
        { timestamp: '10:15 AM', agent: 'Orchestrator Agent', action: 'Analyzing concurrent trade promotion performance', database: 'SAP Scheme Mgmt', query: 'SELECT scheme_name, scheme_type, mtd_spend, cases_generated, gsv_per_rupee FROM active_schemes WHERE region = "North_Region" AND status = "active" ORDER BY gsv_per_rupee DESC', thinking: '4x ROI variance across 7 schemes. Slab mechanics outperform flat margin by 3.2x. Two bottom schemes consuming Rs 1.05Cr/quarter with minimal incremental impact.', nextAgent: 'Sales Agent' },
        { timestamp: '10:17 AM', agent: 'Sales Agent', action: 'Examining scheme-level distributor behavior patterns', database: 'SAP Primary Sales', query: 'SELECT scheme_id, AVG(order_size_change_pct) as behavior_change, COUNT(DISTINCT distributor_id) as participating FROM scheme_participation WHERE region = "North_Region" GROUP BY scheme_id', thinking: 'Slab schemes drive 18% average order size increase — distributors actively optimize to hit thresholds. Flat margin schemes show only 3% order size change — no behavioral shift, just a margin pad on existing orders.', nextAgent: 'Analytics Engine' },
        { timestamp: '10:19 AM', agent: 'Analytics Engine', action: 'Modeling reallocation impact', database: 'Analytics DB', query: 'SELECT reallocation_scenario, projected_incremental_cases, projected_revenue_lift FROM scheme_optimizer WHERE source_schemes IN ("rin_margin", "bpc_bundle") AND target_type = "slab"', thinking: 'Reallocating Rs 50L from flat to slab: projected 3,200 additional cases/month. Same total spend, 28% more output. The ITC share-defense scheme should stay — it is defensive and the 3.4 GSV per Rs spent understates its value (it prevents volume loss).', nextAgent: null }
      ],
      whatIfScenarios: [
        'What if we convert all flat margin schemes to slab-based mechanics?',
        'How would removing the ITC share-defense scheme impact Bareilly volume?',
        'Can we design a combined BPC slab that drives bundle adoption?',
        'What is the optimal slab threshold for Dove-Rin combo?'
      ]
    },
    {
      id: 7,
      ...
      narrative: "Average primary order size per distributor billing has been declining for 5 consecutive months — from Rs 2.24L in Oct 2025 to Rs 1.84L currently. Distributors are placing more frequent, smaller orders rather than consolidated billings. This fragments demand signals and increases HUL's invoice processing load. At Rs 1,840 average drop, distributor net margin is approximately 4.2% vs 6.8% at the LYSM average of Rs 2,240. The decline is concentrated in two factors: (1) retailers splitting orders across more frequent, smaller deliveries to manage working capital, and (2) salesman focus on visit count rather than order value, incentivized by coverage targets rather than revenue targets. Gorakhpur territory maintains Rs 2,380 average drop because salesmen are incentivized on lines-per-order (7.2 avg) rather than just calls-per-day.",
      dataTable: {
    ...
    },
    {
      ...
      recommendation: {
        action: "Add 3 salesmen to Bareilly-Moradabad territory immediately. Recruit from existing HUL bench or temporary hire from local market (Bareilly has adequate talent pool). Deploy with Gorakhpur-style daily audit protocol from day 1. Expected ramp: 4 weeks to full productivity based on existing beat plans.",
        return: "Lifting Bareilly achievement from 76% to 88% = Rs 1.4Cr additional quarterly primary billing",
      ...
      color: C.sage,
      sources: ['shikhar', 'primary'],
      trail: [
        { timestamp: '11:30 AM', agent: 'Orchestrator Agent', action: 'Analyzing territory-level manpower adequacy', database: 'Shikhar App', query: 'SELECT territory, salesman_count, outlet_count, outlets_per_salesman, visit_frequency_pct FROM territory_manpower WHERE region = "North_Region"', thinking: 'Bareilly at 148 outlets/SM — 35% above the 110 benchmark. Every other territory is within 5% of benchmark. This is the most acute manpower imbalance in the region.', nextAgent: 'Sales Agent' },
        { timestamp: '11:32 AM', agent: 'Sales Agent', action: 'Correlating manpower ratio with performance metrics', database: 'Shikhar App', query: 'SELECT territory, outlets_per_salesman, visit_frequency_pct, productive_calls_pct, billing_consistency_pct, achievement_pct FROM territory_performance WHERE region = "North_Region" ORDER BY outlets_per_salesman', thinking: 'Near-perfect inverse correlation (r = -0.94) between outlets-per-salesman and achievement. Every 10 outlets above 110 benchmark correlates with 4-point achievement drop. Bareilly at 148 is 38 outlets above benchmark — predicts 15-point achievement gap, actual gap is 12 points (within model range).', nextAgent: 'Distribution Agent' },
        { timestamp: '11:34 AM', agent: 'Distribution Agent', action: 'Tracing manpower gap to cascading performance impacts', database: 'SAP Primary Sales', query: 'SELECT metric_name, bareilly_value, gorakhpur_value, gap_pct FROM territory_comparison WHERE metrics IN ("visit_frequency", "drop_size", "bpc_distribution", "outlet_service_rate")', thinking: 'Manpower gap cascades: low visit frequency (-22%) leads to low drop size (-32%), which leads to distributor margin compression (-2.6 pts), which leads to BPC deprioritization (-24 pts distribution gap). Fixing manpower is the upstream fix for multiple downstream problems.', nextAgent: 'Analytics Engine' },
  ...
  const territories = [
    {
      ...
      status: 'critical',
      trend: [82, 79, 73, 68],
      topIssue: 'ITC/competitors expanding rapidly, distributor fill rates dropping',
      distributors: [
        {
          name: 'Sharma Trading',
    ...
    },
    {
      ...
      status: 'warning',
      trend: [92, 91, 89, 88],
      topIssue: 'Patanjali gaining in urban MT, Pears/Vim declining',
      distributors: [
        {
    ...
    },
    {
      ...
      topIssue: 'Benchmark performance - replicate display compliance model',
      distributors: [
        {
          name: 'Yadav Distribution',
  ...
  const whatIfResponses = {
    ...
    3: {
      ...
      'What is the revenue correlation between distribution % and summer uplift?': {
        text: "Based on 3 years of UP summer data, the distribution-to-uplift relationship is non-linear:\n\n• 60-70% distribution: 12-15% volume uplift (below threshold for meaningful impact)\n• 70-80% distribution: 18-24% uplift (moderate, but leaves significant money on table)\n• 80-88% distribution: 28-34% uplift (sweet spot - exponential returns)\n• 88%+ distribution: 34-38% uplift (diminishing marginal returns above 88%)\n\nThe critical threshold is 80%. Below it, you're essentially running seasonal peak period at near-normal velocity. Above it, the network effect kicks in - more outlets stocking = more visibility = more consumer trial = more repeat purchases across the seasonal peak window.\n\nAt current 72%, we're firmly in the \"moderate\" zone. Every point of distribution gain between 72-85% delivers approximately ₹6.2L in incremental summer revenue. The economics strongly favor aggressive push to at least 82% (₹62L revenue gain from 10-point improvement, vs approximately ₹8-10L total push cost).",
        suggestions: ['What does the 82% push plan look like?', 'How did top territories achieve 88% last year?', 'Is 85% realistic in 18 days?']
      },
      'Would prioritizing Bareilly and Lucknow territories yield optimal ROI?': {
        text: "Yes - Bareilly and Lucknow represent 84% of the summer distribution gap:\n\n• Moradabad: 64% to target 88% (24-point gap, 288 unbilled outlets)\n• Bareilly: 68% to target 88% (20-point gap, 352 outlets)\n• Combined: 640 of 850 total unbilled seasonal peak outlets (75%)\n\nROI comparison:\n• Bareilly and Moradabad focus: ₹74L revenue protection, ₹6.8L investment = 10.9x ROI\n• Gorakhpur and Eastern focus: ₹14L revenue protection, ₹2.1L investment = 6.7x ROI\n\nBareilly and Moradabad yields 1.6x better ROI because:\n1. Larger outlet universe = more leverage per intervention\n2. Seasonal portfolio supply already at depot (no logistics constraint)\n3. Existing distributor relationships - these aren't new partners, they're underperforming on this specific category\n\nRecommend: 80% of resources to Bareilly and Moradabad, 20% to maintain Gorakhpur and Eastern trajectory. This maximizes the probability of hitting 82%+ UP-wide within the 18-day window.",
        suggestions: ['Build the resource allocation plan', 'What\'s Day 1 action for Moradabad?', 'How do we track daily progress?']
      }
    },
    4: {
      'What is driving the velocity decline at Bareilly flagship outlets specifically?': {
        text: "Cross-referencing SAP billing with Shikhar visit logs at flagship outlets reveals two factors:\n\n1. Visit duration gap: Gorakhpur salesmen spend 8.2 minutes per flagship outlet vs 6.4 minutes in Bareilly. The extra 1.8 minutes likely includes display attention (restocking, rearranging, cleaning). We can see the effect in billing data even though we can't see what happens inside the outlet.\n\n2. SKU breadth at flagship outlets: Gorakhpur flagship outlets average 6.8 billed lines per visit. Bareilly flagship outlets average 4.2 lines. This suggests Gorakhpur salesmen are actively selling the full portfolio at flagship outlets, while Bareilly salesmen are treating flagship outlets the same as any other stop.\n\nThe velocity decline (-14% over 3 months) correlates with the period when Bareilly's outlet-per-salesman ratio crossed 140. Salesmen under time pressure cut the extras first — and flagship outlet attention is an extra.\n\nThe data tells us what's happening. To understand *why* at a process level, we'd need a field investigation: have the ASM ride along with salesmen and observe what Gorakhpur does differently at flagship outlets.",
        suggestions: ['What would a field investigation look like?', 'Can we compare lines-per-order at display vs non-display?', 'Is the decline worse at specific distributor territories?']
      },
      'How much time do Gorakhpur salesmen spend at flagship outlets vs Bareilly?': {
        text: "Shikhar timestamped check-in/check-out data gives us a clear picture:\n\nGorakhpur — flagship outlets:\n• Avg time per visit: 8.2 minutes\n• Avg time at non-display outlets: 6.9 minutes\n• Difference: +19% more time at flagship outlets\n\nBareilly — flagship outlets:\n• Avg time per visit: 6.4 minutes\n• Avg time at non-display outlets: 6.2 minutes\n• Difference: +3% (essentially no difference)\n\nGorakhpur salesmen are clearly treating flagship outlets as higher-value stops that deserve more attention. Bareilly salesmen are not differentiating. The 1.8-minute gap per outlet × 15-20 display visits per day = 27-36 minutes of display-specific activity in Gorakhpur vs essentially zero in Bareilly.\n\nWe can't see from the data exactly what those extra minutes involve. But the billing velocity correlation is clear: more time at flagship outlets = higher throughput at flagship outlets.",
        suggestions: ['How do we get Bareilly salesmen to spend more time at flagship outlets?', 'Would a display-specific incentive change behavior?', 'What does Gorakhpur ASM say about the process?']
      },
      'Would adding new displays in Lucknow outperform fixing velocity on existing Bareilly displays?': {
        text: "Fixing existing Bareilly display velocity is 5x more cost-effective:\n\n• Option A — Recover velocity at 455 existing Bareilly displays:\n  Cost: Primarily execution change (salesman behavior), minimal capex\n  Target: Lift from Rs 9,300 to Rs 11,500/outlet/month (Gorakhpur minus 5% for territory differences)\n  Monthly lift: Rs 10L across 455 outlets\n  Annual impact: Rs 1.2Cr\n\n• Option B — Add 200 new displays in Lucknow:\n  Cost: Rs 24L (Rs 12,000/display placement + logistics)\n  Expected velocity: Rs 10,100/month (current Lucknow display average)\n  Additional vs non-display: Rs 1,300/outlet/month × 200 = Rs 2.6L/month\n  Annual impact: Rs 31.2L\n  Payback period: 9+ months\n\nThe math strongly favors Option A. You already have 455 display assets in Bareilly generating below-potential returns. Every rupee of velocity improvement on those assets is essentially free — you've already paid for the displays.\n\nNew display placement makes sense in Lucknow once Bareilly velocity is recovered, as a growth play rather than a recovery play.",
        suggestions: ['Build the Bareilly velocity recovery plan', 'What is the long-term display placement strategy?', 'How many displays does the region need?']
      },
      'What is the correlation between flagship outlet velocity and competitive billing patterns?': {
        text: "We can see this indirectly through SAP data:\n\nIn the 14 Bareilly beats with suspected competitive displacement (Insight 5), flagship outlets are declining 8% faster than non-display outlets in the same beats. This is counterintuitive — flagship outlets should be *more* resilient because they have branded HUL equipment on-premise.\n\nThis suggests that when flagship outlets are poorly maintained (low velocity, infrequent salesman attention), the display becomes a liability rather than an asset — it signals to the retailer that HUL isn't invested in the outlet, making them more receptive to competitive offers.\n\nConversely, in Gorakhpur (no competitive displacement pattern), flagship outlets are 34% more resilient than non-display outlets during normal volume fluctuations.\n\nThe display is a relationship anchor when it's working. It's a relationship signal when it's not. Right now, in Bareilly, 455 displays are sending the wrong signal.",
        suggestions: ['Should we prioritize display velocity recovery in the competitive beats?', 'How quickly does velocity recovery translate to competitive defense?', 'What is the combined intervention plan?']
      }
    },
    5: {
      'What if this billing decline pattern spreads to Lucknow territory?': {
        text: "Lucknow is the likely next target based on pattern analysis:\n\nThe Bareilly displacement follows a geographic expansion pattern — ITC entered Western UP market through the weakest-served territories first. Lucknow is the logical next step because:\n\n1. Second-lowest salesman visit frequency in the region (84% vs Gorakhpur's 92%)\n2. Larger outlet universe (850 vs Bareilly's 890) with similar mid-tier kirana profile\n3. Adjacent geography — ITC distribution infrastructure built in Bareilly can extend to Lucknow at low marginal cost\n\nIf Lucknow follows the Bareilly trajectory:\n• Expected impact window: 4-8 weeks after first signs appear\n• Revenue exposure: Rs 6.2L/week (Lucknow has higher per-outlet revenue)\n• Most vulnerable: 340 mid-tier outlets with <3x/week visit frequency\n\nPre-emptive action: Increase visit frequency in Lucknow to 3x/week for the top 200 vulnerable outlets now, before displacement starts. Cost: Rs 0.4L/month incremental logistics. This is the cheapest competitive defense available.",
    ...
    6: {
      'What if we convert all flat margin schemes to slab-based mechanics?': {
        text: "Modeling a full conversion of the 2 flat margin schemes (Rin bar 250g, Pears) to slab mechanics:\n\n• Current output: 2,940 incremental cases/month from Rs 92L combined spend (3.2 GSV per Rs spent)\n• Projected output at slab mechanics: 5,800 incremental cases/month from same Rs 92L (6.3 GSV per Rs spent)\n• Net gain: 2,860 additional cases/month = Rs 34L incremental revenue\n\nThe conversion is not risk-free. Flat margin schemes have one advantage: they are simple for distributors to understand and claim. Slab mechanics require distributors to actively manage their ordering to hit thresholds. The 4 distributors in Bareilly with irregular ordering patterns may struggle with slab targets initially.\n\nRecommend: Convert Rin bar 250g first (lower volume, easier to test). Set 3 tiers: 50 cases (Rs 400 bonus), 100 cases (Rs 1,000), 150 cases (Rs 1,800). Keep Pears flat for one more month as control comparison.",
        suggestions: ['What should the Rin slab thresholds be?', 'How do we train distributors on slab mechanics?', 'What if distributors game the system by consolidating orders?']
      },
      'How would removing the ITC share-defense scheme impact Bareilly volume?': {
        text: "Removing the counter-scheme would be catastrophic for Bareilly volume. Here's the projection:\n\nThe counter-scheme currently subsidizes 680 outlets where ITC is actively competing. These outlets generate Rs 4.8L/week in primary billing. The scheme costs Rs 68L MTD (approximately Rs 17L/week).\n\nWithout the scheme:\n• Week 1-2: Distributor margins drop to 2.1% in affected outlets (below breakeven for many)\n• Week 3-4: 40% of affected outlets shift to ITC for at least partial portfolio (based on similar situations in Delhi NCR)\n• Week 8: Estimated 60% volume loss in affected outlets = Rs 2.9L/week lost\n• Quarterly impact: Rs 35L revenue loss\n\nThe counter-scheme's 3.4 GSV per Rs spent ROI looks weak compared to the Surf Excel slab (8.2), but it's defensive — its value is measured in revenue retained, not generated. At Rs 17L/week spend protecting Rs 4.8L/week revenue, the real ROI is: losing the Rs 17L/week spend triggers Rs 2.9L/week revenue loss. Net: you save Rs 17L but lose Rs 2.9L/week forever. Terrible trade.",
      ...
      'Can we design a combined BPC slab that drives bundle adoption?': {
        text: "Yes — this is potentially the highest-impact new scheme we could launch:\n\nConcept: \"Non-Cola Starter Slab\" — Buy 20 cases of any BPC mix (Dove + Rin + Pears), get Rs 1,200 bonus per distributor per week.\n\n• Targets the core problem: BPC is deprioritized because distributors order each SKU separately in small quantities\n• Bundle mechanics force distributors to think about BPC as a single category\n• 20-case threshold is achievable for even mid-tier distributors (current avg is 14 cases/week BPC)\n\nProjected impact:\n• Participating distributors: 32 of 48 (the 16 already above 20 cases don't need the push)\n• Incremental BPC volume: 1,920 cases/month\n• Scheme cost: Rs 19.2L/month\n• GSV per Rs Spent: 5.0 (better than both flat margin schemes currently running)\n\nThis directly addresses the BPC distribution gap (Insight 1) through a scheme-based lever rather than pure execution improvement.",
        suggestions: ['Build the scheme brief for trade marketing', 'Would the Branch Head approve this spend level?', 'How does this interact with existing BPC schemes?']
      },
      'What is the optimal slab threshold for Dove-Rin combo?': {
        text: "Based on current ordering patterns and distributor capacity analysis:\n\nOptimal 3-tier slab:\n• Tier 1: 15 cases (Rs 600 bonus) — targets the 18 distributors currently ordering 10-14 BPC cases/week. Achievable stretch.\n• Tier 2: 25 cases (Rs 1,400 bonus) — targets the 14 distributors currently at 18-22 cases. Meaningful jump.\n• Tier 3: 40 cases (Rs 2,800 bonus) — aspirational. Only 6 distributors currently hit this naturally. But the bonus is attractive enough to motivate serious effort.\n\nWhy these thresholds work:\n1. Tier 1 gap (5-case stretch) is achievable in 1 order cycle — instant wins build momentum\n2. Tier 2 incentive (Rs 1,400) exceeds the margin differential vs core SKUs — makes BPC economically rational\n3. Tier 3 creates aspiration without being unreachable\n\nProjected scheme cost: Rs 22L/month. Projected incremental cases: 2,400/month. GSV per Rs Spent: 5.5 (above the Vim combo scheme at 5.1).",
        suggestions: ['Run this by trade marketing for feasibility', 'How do we prevent cannibalization of existing BPC schemes?', 'What is the minimum participation rate for this to work?']
      }
    },
    7: {
      'What if we implement minimum order thresholds across all territories?': {
  ...
  const sendMessage = (text) => {
    ...
    const insightResponses = {
      1: { // BPC portfolio distribution gap
        'why is this happening?': {
          text: "Three converging factors are driving the 24-point BPC distribution gap:\n\n1. Distributor economics: BPC SKUs (Dove, Rin bar 250g) carry 2-3% lower margin than core SKUs, so distributors systematically deprioritize them in order cycles.\n\n2. Beat coverage gap: Salesmen in Bareilly and Lucknow territories are spending 70% of visit time on core portfolio, leaving insufficient push for BPC. Beat plans don't mandate BPC billing targets.\n\n3. Order size constraints: 4 key distributors have minimum order thresholds that BPC portfolios struggle to meet individually, causing them to be dropped from smaller replenishment cycles.\n\nThe pattern has been building since Oct 2025 - BPC order frequency dropped from 3.2x/month to 2.1x/month at the 4 underperforming distributors.",
        ...
        'what is the risk if we do not act?': {
          text: "The gap is compounding. At current trajectory:\n\n• By end of Feb: Distribution gap widens to 28 points (from 24), adding 380 more unbilled outlets\n• Quarterly revenue loss: Escalates from ₹2.2Cr to ₹2.8Cr\n• Retailer delistings: Once an outlet goes 45+ days unbilled for a SKU, reactivation takes 3x the effort - 285 of the 1,240 outlets are already past 30 days\n• Competitive risk: Outlets unbilled for BPC portfolios are the most vulnerable to competitive substitution. The 14 Bareilly beats already showing billing displacement (Insight 5) overlap significantly with the BPC distribution gap — these outlets are being lost on two fronts simultaneously.\n\nThe tipping point is approximately 3 weeks out - after that, recovery cost doubles.",
          suggestions: ['Show me the action plan', 'What\'s the competitor threat in detail?', 'How do we prevent recurrence?']
        },
        'show me the action plan': {
          text: "Recommended 4-week recovery plan:\n\nWeek 1 (Immediate):\n• Issue combined order mandate - BPC portfolios bundled with core portfolio orders at all 4 distributors\n• Salesman beat plan revision: Mandate minimum 3 BPC portfolio lines per outlet visit\n• Target: 200 outlet reactivations\n\nWeek 2-3 (Acceleration):\n• Outlet-level scheme: ₹40/case incentive for Dove + Rin bar 250g to drive pull-through\n• Dedicated BPC Salesman sweep in Bareilly District (485 unbilled outlets)\n• Daily tracking dashboard shared with distributor principals\n• Target: 600 additional outlet reactivations\n\nWeek 4 (Sustain):\n• Review and lock in permanent combined order threshold\n• Shift Salesman incentive structure to include BPC billing %\n• Target: 84% BPC distribution (from 68%)\n\nInvestment: ₹6.2L (scheme + incremental logistics)\nReturn: ₹1.6Cr quarterly recurring revenue unlock",
          suggestions: ['What are the risks to this plan?', 'Can we accelerate this?', 'Who owns execution?']
        },
        'how do we prevent recurrence?': {
          text: "Three structural changes needed:\n\n1. Combined order mandate (permanent): BPC SKUs included as default line items in every distributor order. System-level enforcement in SAP - orders below BPC threshold flagged automatically.\n\n2. Salesman scorecard revision: Add BPC billing % as a weighted metric (suggest 20% weight). Currently Salesman incentives are 100% volume-based, which naturally favors high-velocity core SKUs.\n\n3. Early warning system: Automated alert when BPC distribution drops below 80% at any distributor for 2 consecutive weeks. Currently the gap built over 8 weeks before surfacing in the morning briefing.\n\nThe Gorakhpur model already implements #1 and #2 - their BPC distribution is at 79% vs 68% region average. Replicating their approach is the lowest-risk path.",
        ...
        default: {
          text: "Looking at the BPC portfolio distribution data, I can help with root cause analysis, action planning, risk quantification, or benchmarking against top-performing territories. What angle would be most useful?",
      ...
      3: { // Seasonal portfolio distribution
        'what if seasonal peak sku distribution remains at 72% through seasonal peak period?': {
          text: "Historical modeling based on last 3 seasonal cycles shows:\n\n• At 72% distribution: Expected volume uplift of 18-20% vs normal period\n• At 88% distribution (target): Expected volume uplift of 34%\n• Revenue delta: ₹88L over the 2-week seasonal peak window\n\nBut the bigger risk is competitive displacement. During Festive season 2025, territories below 75% distribution saw ITC gain 4+ Nielsen share points in seasonal packs — and SAP billing data shows those outlets had not recovered HUL volume 6 months later. Once a retailer stocks ITC for peak season and it sells, the relationship sticks.\n\nBottom line: The ₹88L is the direct loss. The trailing competitive impact adds another ₹30-40L over the following quarter. Total exposure is closer to ₹1.2Cr if we stay at 72%.",
          suggestions: ['What\'s the fastest path to 82%?', 'Which outlets matter most for peak season?', 'How did we hit 88% last year?']
        },
        'why is this happening?': {
          text: "The seasonal peak SKU distribution lag has two root causes:\n\n1. Late pipeline activation: Seasonal portfolio primary billing push started 2 weeks later than last year. In 2025, distributor stocking began 30 days before peak. This year, it started at 18 days. The 12-day delay directly translates to the 6-point gap vs LYSM.\n\n2. Bareilly and Lucknow distributor deprioritization: These territories are dealing with the BPC portfolio gap simultaneously. Distributors are capacity-constrained on order processing - they're managing core + BPC catch-up, and seasonal peak SKUs are getting squeezed out of the order cycle.\n\nThe good news: Supply is not a constraint. Depot inventory is at 142% of requirement. This is purely a distribution execution problem with a known solution - accelerated primary loading push and distributor-level activation.",
          suggestions: ['Show me the action plan', 'Can we prioritize seasonal peak over BPC for 2 weeks?', 'What is the daily distribution gain needed?']
        },
        default: {
          ...
          suggestions: ['What if seasonal peak SKU distribution remains at 72% through seasonal peak period?', 'Show me the action plan']
        }
      },
      4: { // Flagship outlet velocity (positive)
        'why is this happening?': {
          text: "The data shows *what's* happening clearly; the *why* needs field investigation:\n\nWhat we can see from SAP + Shikhar:\n• Gorakhpur flagship outlets: Rs 12,200/month velocity, stable 6 months. Salesmen spend 8.2 min per flagship outlet.\n• Bareilly flagship outlets: Rs 9,300/month velocity, down 14% in 3 months. Salesmen spend 6.4 min per flagship outlet.\n• The velocity gap is display-specific — non-display outlets in both territories show comparable trends.\n\nWhat we can infer: Gorakhpur salesmen are doing something different at flagship outlets that Bareilly salesmen are not. The extra 1.8 minutes per visit correlates with 28% higher billing and higher lines-per-order (6.8 vs 4.2).\n\nWhat we can't see from data alone: What exactly happens during those extra minutes. Is it display restocking? SKU arrangement? Retailer conversations about the full portfolio? This is where ASM Gorakhpur's input becomes critical — he reports that his team pays specific attention to display presentation during visits, but we'd need a field ride-along to document the exact process.",
        ...
        'show me the action plan': {
          text: "Two-phase approach — investigate, then replicate:\n\nPhase 1: Understand (Week 1-2)\n• ASM Gorakhpur ride-along: 2 days observing top-performing salesmen at flagship outlets, documenting the process\n• Compare visit pattern data: pull Shikhar time-per-visit, lines-per-order, and SKU mix at display vs non-display outlets across all territories\n• Interview ASM Gorakhpur on the process changes he implemented and when velocity started improving\n\nPhase 2: Pilot (Week 3-6)\n• Deploy Gorakhpur's process (once documented) to 3 Bareilly salesmen covering ~230 flagship outlets\n• Track weekly: flagship outlet velocity, lines-per-order, visit duration\n• Compare pilot salesmen vs control (other 3 Bareilly salesmen)\n• If velocity improves, roll to all Bareilly salesmen in week 6, then Lucknow\n\nCost: Minimal (primarily ASM time for investigation + ride-along). No capex.\nExpected outcome: If Bareilly display velocity recovers to Rs 11,500 (Gorakhpur minus territory adjustment), monthly uplift = Rs 10L across 455 outlets.",
          suggestions: ['How do we measure the pilot?', 'Who runs the ride-along?', 'What if the process is too complex to replicate?']
        },
        default: {
          text: "Gorakhpur's flagship outlets are significantly outperforming the rest of the region. The data shows what's happening — billing velocity, visit duration, lines-per-order all correlate. I can walk you through the data, the investigation plan, or the revenue opportunity. What's most useful?",
      ...
      5: { // Competitive displacement (billing pattern)
        ...
        'show me the action plan': {
          ...
          suggestions: ['Deploy the retention scheme today', 'How do we monitor Lucknow?', 'What if the field audit confirms ITC?']
        },
        default: {
          text: "SAP primary data has flagged an unusual billing decline pattern in 14 Bareilly beats — synchronized, geographically clustered, and concentrated in mid-tier outlets. The pattern is consistent with competitive displacement but needs field confirmation. I can walk you through the data, the investigation plan, or the containment strategy. What's most urgent?",
          suggestions: ['Why is this happening?', 'Show me the action plan', 'Which outlets are declining fastest?']
        }
      },
      6: { // Trade promo ROI
        'why is this happening?': {
          text: "The 4x ROI variance traces to a fundamental difference in scheme mechanics:\n\nSlab schemes (8.2 GSV per Rs spent) change behavior: Distributors actively optimize ordering to hit thresholds. The Surf Excel slab (buy 10 get 1 free) creates a target that distributors plan around. They consolidate orders, push salesmen to fill gaps, and manage inventory to hit the next tier.\n\nFlat margin schemes (2.1 GSV per Rs spent) subsidize status quo: A flat Rs 8/case on Rin doesn't change how distributors order. They claim the margin on whatever they would have ordered anyway. There's no threshold to chase, no behavior to change.\n\nThe evidence is clear in order pattern data: Slab scheme distributors show 18% larger average orders and 12% more frequent ordering. Flat margin distributors show zero change in ordering pattern — the same cadence at a lower cost to HUL.\n\nWe're essentially paying Rs 50L/month (Rin + BPC bundle) for zero incremental behavior change.",
          suggestions: ['How do we fix underperforming schemes?', 'Show me the action plan', 'What slab mechanics would work for BPC?']
        },
        default: {
      ...
      8: { // Manpower
        ...
        'show me the action plan': {
          ...
          suggestions: ['How do we get the Branch Head to approve 3 hires?', 'What is the risk if we delay?', 'Build the business case document']
        },
        default: {
  ...
  const morrieResponses = {
    0: [
      { q: 'Which specific outlets are most at risk?', a: 'The 820 at-risk outlets break down as: 380 outlets served by Sharma Trading (Bareilly City), 240 by Gupta Sales (Rampur), and 200 by Verma Agencies (Bareilly South). Sharma\'s outlets are most critical — their Surf Excel 1kg pack stock is at 0.8 days cover vs 1.2 days average. These 380 outlets account for Rs 2.4L/day of the Rs 5.2L daily exposure.' },
      { q: 'What\'s the shortest reorder path?', a: 'Direct restock from the regional depot is the fastest route — 18-hour turnaround vs 36 hours through the standard reorder cycle. The depot currently has 6,800 cases of Surf Excel 1kg pack and 4,100 cases of Vim 500g pack in finished goods inventory. The depot is 22 km from Sharma Trading and 35 km from Gupta Sales — well within same-day dispatch range. Recommend triggering an emergency depot-to-distributor dispatch for all 3 distributors by EOD today. Logistics cost: ₹18K for direct depot dispatch vs ₹4.2L/day in sales loss if stockout hits. The depot route adds an extra 18 hours because stock has to move factory → depot → distributor instead of depot → distributor directly.' },
      { q: 'Has this happened before with these distributors?', a: 'Yes — Sharma Trading had a similar stockout event in Oct 2025 (lasted 4 days, Rs 17L revenue loss on Home Care SKUs). Root cause was the same: ordering rhythm dropped from 3x/week to 1x/week without early detection. Post-incident, a reorder trigger was set at 2.5 days cover but was later relaxed to 1.5 days. Recommend resetting threshold to 2.5 days permanently.' }
    ],
    1: [
      { q: 'What\'s the risk timeline if we don\'t act?', a: 'At current depletion rate, Sharma Trading hits zero Surf Excel 1kg stock in 3-4 days, Verma Agencies and Gupta Sales in 5-6 days (they have slightly more cover). Once stockout occurs, outlet reactivation takes 5-7 days on average (retailers switch to ITC or local brands). Projected cascade: 820 outlets affected, Rs 5.2L/day exposure. The reorder needs to be triggered within 48 hours to prevent the gap.' },
      { q: 'Can we redirect stock from other territories?', a: 'Gorakhpur currently holds 142% of requirement for Dove 180ml bottle - Yadav Distribution has 680 excess cases. Inter-distributor transfer to Bareilly City is feasible within 24 hours at ₹8K logistics cost. This buys 3.5 days additional cover while the standard reorder cycle catches up. Recommend authorizing the transfer and expediting primary reorder simultaneously.' }
    ],
    2: [
      { q: 'What changed in the last 10 days?', a: 'SAP data shows the drop started Jan 19 - coinciding with a beat restructuring in Bareilly territory. 3 of the 4 distributors (Sharma Trading, Verma Agencies, Gupta Sales) had their Salesman routes reorganized. Order pattern analysis shows Salesmen are spending 40% more time on new routes (learning curve) and placing smaller, less frequent orders. Singh Distributors\' drop is unrelated - their primary salesperson has been on leave since Jan 18.' },
      { q: 'What\'s the revenue impact so far?', a: 'The 32% order frequency drop translates to ₹14.2L in missed primary billing this week across 1,140 outlets. At current trajectory, this becomes ₹28.4L over two weeks. The beat restructuring should normalize in 7-10 days based on historical patterns, but Singh Distributors needs immediate Salesman coverage intervention - their single-point-of-failure staffing model is the structural risk here.' }
    ],
    3: [
      { q: 'Where exactly is the Rs 4.0Cr gap coming from?', a: 'Breaking down by territory: Bareilly accounts for Rs 2.1Cr of the gap (at 72% achievement), Lucknow for Rs 0.9Cr (at 84%), and the remaining Rs 1.0Cr is spread across Ayodhya and minor shortfalls. Gorakhpur territory is actually over-indexing at 104% - without their performance, the gap would be Rs 5.4Cr. The Bareilly gap is almost entirely distributor-execution driven (order frequency and outlet coverage), not demand-side.' },
    ...
    4: [
      { q: 'What drove the recovery?', a: 'Three factors converged: (1) A focused "Operation Restock" initiative by the ASM - dedicated 3 Salesman man-days per distributor over 2 weeks to reactivate dropped outlets. (2) Verma Agencies increased their order frequency from 1.5x to 3x per week after a stock guarantee commitment from the depot. (3) Outlet-level scheme (₹50 per case incentive for Dove) drove pull-through demand. The scheme cost ₹1.2L but generated ₹8.4L in incremental billing.' },
    ...
    <div className='overflow-auto' style={{ height: '100vh', backgroundColor: C.offWhite, fontFamily: font, display: 'flex' }}>
      ...
      <div style={{ flex: 1, marginLeft: sidebarWidth, transition: 'margin-left 0.2s ease', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ 
          ...
          {/* Left: questt x HUL + Sales Watchtower */}
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            ...
            {/* HUL branding */}
            <span style={{ fontSize: "12px", fontWeight: "600", color: C.sage, fontFamily: font, letterSpacing: "0.02em" }}>HUL</span>
              ...
              {profileDropdownOpen && (
                <div onClick={e => e.stopPropagation()} style={{ position: 'absolute', top: '100%', right: 0, marginTop: '8px', width: '220px', backgroundColor: C.white, border: `1px solid ${C.cream}`, borderRadius: '8px', boxShadow: '0 8px 30px rgba(0,0,0,0.1)', overflow: 'hidden', zIndex: 200 }}>
                  <div style={{ padding: '14px 16px', borderBottom: `1px solid ${C.cream}` }}>
                    <div style={{ fontSize: '13px', fontWeight: '600', color: C.green }}>Rajesh Mehta</div>
                    <div style={{ fontSize: '11px', color: C.lightGrey, marginTop: '2px' }}>rajesh.mehta@hul.co.in</div>
                  </div>
                  {[
          ...
          {activeTab === 'briefing' && (
            <>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '32px' }}>
                {[
                  { label: 'Primary Billing MTD', value: '₹18.0Cr', change: '82% of ₹22.0Cr target', trend: 'down', icon: Target, color: C.sage },
                  { label: 'Active Distributors', value: '15', sub: 'of 15 total', icon: MapPin, color: C.sage },
                  { label: 'Avg Order Freq.', value: '3.1/mo', change: '-16% vs LYSM', trend: 'down', icon: Activity, color: C.lightGrey },
                  { label: 'SKU Breadth', value: '28 avg', sub: 'vs 34 Gorakhpur', icon: Package, color: C.sage }
                ].map((metric, i) => (
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: '8px', marginBottom: '24px', overflowX: 'auto', paddingBottom: '4px', flexWrap: 'wrap' }}>
                  {[
                    ...
                    { key: 'bpc', label: 'BPC vs Cola', count: insights.filter(i => i.category === 'bpc').length },
                    ...
                    { key: 'display', label: 'Display Asset Performance', count: insights.filter(i => i.category === 'display').length },
          ...
          {activeTab === 'deepdive' && (
            <div>
              <div style={{ marginBottom: '32px' }}>
                ...
                <p style={{ fontSize: '14px', color: C.lightGrey, margin: 0 }}>
                  Deep dive into North India region ASM territories and their distributor networks
                </p>
              </div>
                  ...
                  >
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginTop: '20px' }}>
                      {[
                        ...
                        { label: 'BPC Mix', value: `${territory.mustSellMix}%`, good: territory.mustSellMix >= 42 },
        ...
        <div style={{ marginTop: 'auto', borderTop: `1px solid ${C.cream}`, padding: '12px 32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: C.white }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
            ...
            <span style={{ fontSize: '10px', fontWeight: '600', color: C.darkGrey, fontFamily: serif }}>HUL</span>
                  ...
                  <div style={{ padding: '18px', backgroundColor: C.offWhite, borderRadius: '6px', border: `1px solid ${C.cream}` }}>
                    ...
                    <div style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '6px 10px', backgroundColor: C.lightSage, borderRadius: '4px', fontSize: '11px', fontWeight: '600', color: C.sage, marginBottom: '10px' }}>
                      ...
                      <span style={{ fontSize: '9px', color: C.lightGrey, fontWeight: '400' }}>
                        {step.database === 'SAP Primary Sales' ? 'T+1' : step.database === 'Shikhar App' ? 'T+0' : step.database === 'Nielsen IQ' ? 'T+6w' : 'T+1'}
                      </span>
                    </div>
