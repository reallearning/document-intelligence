...
export default function BuyPlan(){
    ...
    return (<div onClick={()=>{setScenario(null);setReason("");}} style={{position:"fixed",inset:0,zIndex:100,background:"rgba(27,42,33,.4)",display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:12,width:"100%",maxWidth:580,boxShadow:"0 24px 80px rgba(12,44,24,.18)",maxHeight:"90vh",overflow:"auto",border:`1px solid ${C.border}`}}>
        <div style={{padding:"22px 28px",background:C.bgSub,borderRadius:"12px 12px 0 0",borderBottom:`1px solid ${C.border}`}}>
          ...
          <div style={{fontSize:13,color:C.dim,marginTop:6}}>OTB impact: <b style={{color:diff>0?C.green:C.terra}}>{diff>0?"+":""}₹{valImpact}L</b> · MRP ₹{s.mrp.toLocaleString()} · {s.margin}% margin</div>
        </div>
  ...
  const Header=()=> (<div style={{background:C.green,color:"#fff",padding:"16px 32px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
    <div style={{display:"flex",alignItems:"center",gap:16}}><div style={{display:"flex",flexDirection:"column",alignItems:"flex-start"}}><div style={{display:"flex",alignItems:"center"}}><span style={{fontSize:15,fontWeight:500,letterSpacing:0,fontFamily:font,opacity:.85}}>questt</span><span style={{color:"#3BB896",fontSize:15,fontWeight:700}}>.</span></div><span style={{fontSize:9,opacity:.45,fontWeight:400,letterSpacing:.3,marginTop:-1}}>powered by PwC</span></div><span style={{opacity:.25,margin:"0 10px"}}>|</span><span style={{fontSize:14,opacity:.6,fontWeight:400}}>Buy Planning · SS26</span></div>
    {view==="plan"&&<div style={{display:"flex",alignItems:"center",gap:16}}>
      <div style={{padding:"5px 10px",background:"rgba(255,255,255,.08)",borderRadius:4,fontSize:11,color:"rgba(255,255,255,.6)"}}>Target: 6-wk ST above 90%</div>
    </div>}
  </div>);
        ...
        {Object.keys(s.hist||{}).length>0&&<div style={{marginTop:12}}>
          ...
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
            <thead><tr style={{borderBottom:`2px solid ${C.border}`}}>
              {["Season","Units","6-wk ST","vs Target","Margin","Wks to Clear","Notes"].map(h=> (<th key={h} style={{textAlign:"left",padding:"8px 8px",fontSize:12,fontWeight:600,color:C.dim}}>{h}</th>))}
            </tr></thead>
            <tbody>{Object.entries(s.hist).sort((a,b)=>b[0].localeCompare(a[0])).map(([ssn,d])=> (<tr key={ssn} style={{borderBottom:`1px solid ${C.borderLight}`}}>
              ...
              <td style={{padding:"8px 8px",fontSize:13}}>{d.m}%</td>
  ...
  const SeasonsHome=()=>{
    ...
    return (<div className="overflow-auto" style={{height:"100vh",background:C.bg,fontFamily:font}}>
      ...
      <div style={{background:C.green,color:"#fff",padding:"16px 32px",display:"flex",alignItems:"center"}}><div style={{maxWidth:1100,margin:"0 auto",width:"100%",display:"flex",alignItems:"center",gap:12}}><div style={{display:"flex",flexDirection:"column",alignItems:"flex-start"}}><div style={{display:"flex",alignItems:"center"}}><span style={{fontSize:15,fontWeight:500,letterSpacing:0,fontFamily:font,opacity:.85}}>questt</span><span style={{color:"#3BB896",fontSize:15,fontWeight:700}}>.</span></div><span style={{fontSize:9,opacity:.45,fontWeight:400,letterSpacing:.3,marginTop:-1}}>powered by PwC</span></div><span style={{opacity:.25,margin:"0 10px"}}>|</span><span style={{fontSize:14,opacity:.6,fontWeight:400}}>Buy Planning</span></div></div>
      <div style={{maxWidth:1100,margin:"0 auto",padding:"36px 32px 64px"}}>
        <div onClick={()=>{setView("plan");setSeason("SS26");}} style={{background:`linear-gradient(135deg, ${C.green} 0%, #1A3D28 100%)`,borderRadius:14,padding:"36px 36px 32px",cursor:"pointer",color:"#fff",marginBottom:32,position:"relative",overflow:"hidden"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}><span style={{fontSize:12,padding:"4px 12px",borderRadius:20,fontWeight:600,background:"rgba(223,118,73,0.25)",color:"#F8A87A",border:"1px solid rgba(223,118,73,0.3)"}}>{seasons[0].status}</span></div>
          <div style={{fontSize:28,fontWeight:700,fontFamily:serif,marginBottom:4}}>{seasons[0].label}</div>
          <div style={{fontSize:14,opacity:.7,marginBottom:24}}>{seasons[0].extra}</div>
          <div style={{display:"flex",gap:40,marginBottom:28}}>{[{label:"Styles",value:seasons[0].styles},{label:"Total Units",value:seasons[0].units},{label:"OTB Budget",value:seasons[0].budget},{label:"Avg Margin",value:seasons[0].margin}].map(m=> (<div key={m.label}><div style={{fontSize:11,opacity:.5,fontWeight:500,letterSpacing:.5}}>{m.label}</div><div style={{fontSize:20,fontWeight:700,marginTop:3}}>{m.value}</div></div>))}</div>
          <div style={{display:"inline-flex",alignItems:"center",gap:6,padding:"10px 22px",background:"rgba(255,255,255,0.15)",borderRadius:8,border:"1px solid rgba(255,255,255,0.2)",fontWeight:600,fontSize:14,backdropFilter:"blur(4px)"}}>Finalize Buy Plan <span style={{fontSize:16}}>→</span></div>

        </div>

        <div style={{fontSize:12,fontWeight:600,color:C.dim,letterSpacing:1,marginBottom:12}}>PAST SEASONS</div>
        <div style={{borderRadius:10,border:`1px solid ${C.border}`,overflow:"hidden"}}>
          {seasons.slice(1).map((s,i)=> (<div key={s.id} onClick={()=>{setView("past");setSeason(s.id);setPastTab("style");}} style={{padding:"18px 24px",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"space-between",borderBottom:i<seasons.length-2?`1px solid ${C.borderLight}`:"none",background:i%2===0?C.bg:C.bgSub,transition:"background 0.15s"}}>
            <div style={{display:"flex",alignItems:"center",gap:14}}>
              ...
              <div><div style={{fontSize:14,fontWeight:500,color:C.text}}>{s.label}</div><div style={{fontSize:12,color:C.dimmer,marginTop:1}}>{s.extra}</div></div>
            </div>
  ...
  const PastSeasonView=()=>{
    ...
    return (<div className="overflow-auto" style={{height:"100vh",background:C.bg,fontFamily:font}}>
      <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800&family=DM+Serif+Display:ital@0;1&display=swap" rel="stylesheet"/>
      <div style={{background:C.green,color:"#fff",padding:"16px 32px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div style={{display:"flex",alignItems:"center",gap:12}}><div style={{display:"flex",flexDirection:"column",alignItems:"flex-start"}}><div style={{display:"flex",alignItems:"center"}}><span style={{fontSize:15,fontWeight:500,letterSpacing:0,fontFamily:font,opacity:.85}}>questt</span><span style={{color:"#3BB896",fontSize:15,fontWeight:700}}>.</span></div><span style={{fontSize:9,opacity:.45,fontWeight:400,letterSpacing:.3,marginTop:-1}}>powered by PwC</span></div><span style={{opacity:.25,margin:"0 10px"}}>|</span><span style={{fontSize:14,opacity:.7}}>Buy Planning · {season}</span></div>
        <div style={{display:"flex",alignItems:"center",gap:16}}>
          <span style={{fontSize:12,padding:"3px 10px",borderRadius:20,fontWeight:600,background:"rgba(255,255,255,.12)",color:"rgba(255,255,255,.8)"}}>{ps.status}{ps.week!=="Final"?` · ${ps.week}`:""}</span>
          <span style={{fontSize:13}}>Avg ST: <b>{ps.stAvg}</b></span>
        </div>
      </div>
      <div style={{maxWidth:1100,margin:"0 auto",padding:"28px 32px"}}>
        ...
        <div style={{fontSize:13,color:C.dim,marginTop:3}}>{ps.styles} styles · {ps.units} units · {ps.budget} · {ps.margin} margin</div>
  ...
  return (<div className="overflow-auto" style={{height:"100vh",background:C.bg,fontFamily:font}}>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800&family=DM+Serif+Display:ital@0;1&display=swap" rel="stylesheet"/>
    <Header/><ScenarioModal/><TrailModal/><StoreEditModal/>
