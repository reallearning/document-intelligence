...
import { uploadFileLivspace } from "@/services/upload-file";
...
const UploadPage = () => {
  const { saveLivspaceData } = useDocumentData();
  ...
  const handleFileUpload = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/upload/livspace", {
        method: "POST",
        body: formData,
      });
      ...
      const { data: highlightsData } = await uploadFileLivspace({
        doc_url: pdfUrl,
      ...
      saveLivspaceData(highlightsData);
      setFileState((prevState) => ({
        ...prevState,
      ...
      router.push("/demo/livspace/details");
    } catch (error) {
